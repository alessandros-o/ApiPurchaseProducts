...
public class PurchasesService {
    ...
    public ResponseBase<PurchaseResponse> cadastrar(PurchaseCreateRequest novaCompra) {
        ...
        for (PurchaseItem item : novaCompra.getItens()) {
            ...
            // Verifica se o product existe e caso contrario retorna um erro
            ...
            // Verifica se tem desconto, se tiver inicia calculo
    ...
    public ResponseBase<Page<PurchaseResponse>> pesquisar(SearchOrdersRequest searchOrdersRequest){
        if (searchOrdersRequest.getPaginaAtual() < 0){
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "O indice da página atual deve começar em 0");
        }

        if (searchOrdersRequest.getQtdPorPagina() < 1 || searchOrdersRequest.getQtdPorPagina() > 50){
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Quantidade de itens por página deve ser entre 1 e 50 itens");
        }

        PageRequest pageRequest = PageRequest.of(searchOrdersRequest.getPaginaAtual(), searchOrdersRequest.getQtdPorPagina());
        Page<Order> paginatedResponse;

        Double minPrice = searchOrdersRequest.getMinPrice();
        if (minPrice != null){
